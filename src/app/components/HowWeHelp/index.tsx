  ...
  const features = [
    {
      ...
      title: "Divórcio Amigável e Litigioso",
      desc: "Guiamos você através do processo de divórcio com compreensão e profissionalismo, buscando soluções eficientes para proteger seus interesses",
    },
    {
      icon: (
        <svg
    ...
    },
    {
      ...
      title: "Adoção e Questões de Paternidade",
      desc: "Assistimos em processos de adoção e questões de paternidade, garantindo que todos os procedimentos legais sejam seguidos para criar lares amorosos e estáveis",
    },
  ];
          ...
          <div className="relative mt-6">
            <ul className="grid gap-8 sm:grid-cols-1 lg:grid-cols-3">
              {features.map((item, idx) => (
                <li
                  key={idx}
                  className="bg-cyan-950 space-y-3 p-4 rounded-lg shadow-inner shadow-cyan-800 hover:transition-all"
                >
                  <div className="flex items-center justify-center">
                    <div className="text-white">{item.icon}</div>
                  </div>

                  <h1 className="p-4 text-center text-lg font-extrabold text-white">
                    {item.title}
                  </h1>
